...
export function QuizTake({ quiz }: { quiz: QuizApiPayload }) {
  ...
  async function handleStart() {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        alert(data.error ?? "تعذر بدء الاختبار");
        return;
      }
    ...
    <div className="mt-8 space-y-8">
      ...
      {!started && !submitted ? (
        <div className="rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
          <h3 className="text-lg font-semibold text-[var(--color-foreground)]">هل أنت جاهز لبدء الاختبار؟</h3>
          <p className="mt-2 text-sm text-[var(--color-muted)]">
            عند الضغط على «ابدأ الاختبار» سيتم احتساب محاولة من العدد المسموح.
            {maxQuizAttempts != null && attemptsUsed != null ? (
              <span className="mr-1"> (المستخدم: {attemptsUsed} من {maxQuizAttempts})</span>
            ) : null}
          </p>
          {!canAttempt ? (
            <p className="mt-4 rounded-[var(--radius-btn)] border border-red-500/40 bg-red-500/10 px-4 py-3 text-sm text-red-600 dark:text-red-400">
              لا يمكنك بدء محاولة جديدة لهذا الاختبار بسبب حد المحاولات.
            </p>
          ) : null}
